...
export default function Layout() {
  ...
  const handleSendMagicLink = async (e) => {
    ...
    try {
      ...
      setMailError(lang === 'en' ? 'Error sending email. Try again.' : lang === 'cat' ? 'Error enviant el correu. Torna a provar.' : 'Error al enviar el email. Inténtalo de nuevo.');
    } finally {
      setSending(false);
    }
  };
  ...
  const openModal = () => { setShowModal(true); setMailSent(false); setEmail(''); setMailError(''); };
  const closeModal = () => setShowModal(false);

  const profileLabel = lang === 'en' ? 'My profile' : lang === 'cat' ? 'El meu perfil' : 'Mi perfil';
  const signOutLabel = lang === 'en' ? 'Sign out' : lang === 'cat' ? 'Sortir' : 'Salir';
  const modalTitle   = lang === 'en' ? 'Enter your email' : lang === 'cat' ? 'Introdueix el teu correu' : 'Introduce tu email';
  const modalSub     = lang === 'en' ? "We'll send you a magic link to sign in instantly — no password needed." : lang === 'cat' ? "T'enviarem un enllaç màgic per entrar al instant — sense contrasenya." : 'Te enviaremos un enlace mágico para entrar al instante — sin contraseña.';
  const sentTitle    = lang === 'en' ? 'Check your email!' : lang === 'cat' ? 'Revisa el teu correu!' : '¡Revisa tu email!';
  const sentSub      = lang === 'en' ? 'We sent a magic link to' : lang === 'cat' ? "Hem enviat un enllaç màgic a" : 'Hemos enviado un enlace mágico a';
  const sendLabel    = lang === 'en' ? 'Send magic link' : lang === 'cat' ? 'Enviar enllaç màgic' : 'Enviar enlace mágico';
        ...
        <div onClick={closeModal} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(12,12,11,0.85)', backdropFilter: 'blur(8px)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div onClick={e => e.stopPropagation()} style={{ backgroundColor: colors.surface, border: `1px solid ${colors.borderMedium}`, borderRadius: '8px', padding: 'clamp(28px,5vw,40px)', maxWidth: '400px', width: '100%', display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            {!mailSent ? (
              <>
                <div>
                  <h2 style={{ fontFamily: fonts.display, fontWeight: 900, fontSize: 'clamp(18px,3vw,22px)', textTransform: 'uppercase', letterSpacing: '-0.02em', color: colors.textPrimary, marginBottom: '8px', textAlign: 'center' }}>{modalTitle}</h2>
                  <p style={{ fontFamily: fonts.mono, fontSize: '12px', color: colors.textSecondary, textAlign: 'center', lineHeight: 1.6 }}>{modalSub}</p>
                </div>

                {mailError && (
                  <div style={{ fontFamily: fonts.mono, fontSize: '12px', color: colors.error, backgroundColor: 'rgba(255,68,68,0.08)', border: '1px solid rgba(255,68,68,0.2)', borderRadius: '4px', padding: '10px 12px' }}>{mailError}</div>
                )}

                <form onSubmit={handleSendMagicLink} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  <input
                    type="email" value={email} onChange={e => setEmail(e.target.value)}
                    placeholder="tu@email.com" required autoFocus
                    style={{ fontFamily: fonts.mono, fontSize: '14px', padding: '12px', backgroundColor: colors.background, border: `1px solid ${colors.border}`, borderRadius: '4px', color: colors.textPrimary, outline: 'none', transition: 'border-color 0.2s' }}
              ...
              <>
                ...
                <div>
                  <h2 style={{ fontFamily: fonts.display, fontWeight: 900, fontSize: 'clamp(18px,3vw,22px)', textTransform: 'uppercase', color: colors.accent, marginBottom: '8px', textAlign: 'center' }}>{sentTitle}</h2>
                  <p style={{ fontFamily: fonts.mono, fontSize: '12px', color: colors.textSecondary, textAlign: 'center', lineHeight: 1.6 }}>
                    {sentSub} <strong style={{ color: colors.textPrimary }}>{email}</strong>.
                    <br />
                    {lang === 'en' ? 'Click the link to sign in.' : lang === 'cat' ? "Fes clic a l'enllaç per entrar." : 'Haz clic en el enlace para entrar.'}
                  </p>
                </div>
                <button onClick={closeModal} style={{ fontFamily: fonts.mono, fontWeight: 700, fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.1em', padding: '12px', border: `1px solid ${colors.accent}`, backgroundColor: 'transparent', color: colors.accent, borderRadius: '4px', cursor: 'pointer' }}>
                  {lang === 'en' ? 'Close' : lang === 'cat' ? 'Tancar' : 'Cerrar'}
                </button>
              </>
            )}
          </div>
                ...
                user ? (
                  <>
                    <button onClick={() => navigate('/perfil')}
                      ...
                      onMouseEnter={e => { e.currentTarget.style.backgroundColor = colors.accentGlow || 'rgba(0,255,135,0.08)'; }}
